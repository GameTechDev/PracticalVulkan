...
//  |  Added the below function definition
// \ /
...
//   "%5.2f secs"   01.23 secs
//   "Gold: %.0f"   Gold: 1
bool ImGui::SliderFloatWithStep(const char* label, float* v, float v_min, float v_max, float v_step, const char* display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    float remain = roundf(fmodf(*v, v_step) / v_step);
    float val = (remain + ((int)*v / (int)v_step)) * v_step;
    *v = fmaxf(v_min, val);
...
//  |  Added the below function declaration
// \ /
bool ImGui::SliderIntWithStep(const char* label, int* v, int v_min, int v_max, int v_step, const char* display_format)
{
    if (!display_format)
        display_format = "%.0f";
    float v_f = (float)*v;
    bool value_changed = SliderFloatWithStep(label, &v_f, (float)v_min, (float)v_max, (float)v_step, display_format, 1.0f);
    *v = (int)v_f;
    return value_changed;
}
