...
namespace ImGui
{
    ...
    // \ /
    IMGUI_API bool          SliderFloatWithStep(const char* label, float* v, float v_min, float v_max, float v_step, const char* display_format = "%.3f", float power = 1.0f);     // adjust display_format to decorate the value with a prefix or a suffix for in-slider labels or unit display. Use power!=1.0 for logarithmic sliders
    ...
    //  |  Added the below function declaration
    // \ /
    IMGUI_API bool          SliderIntWithStep( const char* label, int* v, int v_min, int v_max, int v_step, const char* display_format = "%.0f" );
